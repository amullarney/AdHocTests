...
public class Client extends Component<Client> {
    ...
    public Client(IApplication app, IRunContext runContext, int populationId) {
        ...
        classDirectory.put("EmployeeMenu", EmployeeMenuImpl.class);
    }
    ...
    public void T2() throws XtumlException {
        context().LOG().LogInfo( "T2 invoked in Client" );
        EmployeeSet ees = context().Employee_instances();
        context().Srv().c( (sysconfig.hr.EmployeeSet)ees );
    }
